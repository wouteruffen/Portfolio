  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background text-foreground">
        ...
        <SubpageHeader title={pp.pageTitle} />
          ...
          <div className={SECTION_TITLE_CONTAINER_CLASS}>
            <div className="grid md:grid-cols-2 gap-16">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                <h2 className="text-3xl font-antonio font-semibold mb-6">{pp.heading}</h2>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}>
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={projectProduct} alt={pp.imageAlt} className="w-full h-full object-cover" loading="lazy" width={800} height={600} />
                </div>
              </motion.div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mt-24">
              <h2 className="text-xs tracking-[0.3em] text-muted-foreground mb-10 font-body uppercase">{pp.expertiseTitle}</h2>
              <div className="grid md:grid-cols-2 gap-6">
                {pp.expertise.map((item) => (
